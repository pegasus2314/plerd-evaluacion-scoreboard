...
async function loadCenters(model) {
  ...
  const { data, error } = await supabase
    ...
    .order('nombre');
  if (error) throw error;
  select.innerHTML = '<option value="">Selecciona un centro…</option>' + (data || []).map(c => `<option value="${esc(c.id)}">${esc(c.nombre)}${c.nivel ? ` · ${esc(c.nivel)}` : ''}</option>`).join('');
}
...
function renderRows(rows) {
  ...
  list.querySelectorAll('[data-center-stage]').forEach(select => {
    select.addEventListener('change', async e => updateRow(e.target.dataset.centerStage, { etapa: e.target.value }));
  });
  list.querySelectorAll('[data-center-state]').forEach(select => {
    select.addEventListener('change', async e => updateRow(e.target.dataset.centerState, { estado: e.target.value }));
  });
...
async function subscribe(modelId) {
  ...
  if (!modelId) return;
  channel = supabase.channel(`model-centers-${modelId}`)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'scoreboard_model_centers', filter: `model_id=eq.${modelId}` }, () => loadAssignments().catch(console.error))
    .subscribe();
}
...
function start() {
  const observer = new MutationObserver(() => {
    if (document.querySelector('.r17-admin-grid')) render().catch(console.error);
  });
  observer.observe(document.body, { childList: true, subtree: true });
  setInterval(() => { if (document.querySelector('.r17-admin-grid')) render().catch(console.error); }, 1200);
  window.addEventListener('storage', e => { if (e.key === 'r17:modelId') { lastModelId = null; render().catch(console.error); } });
