...
import { BarChart3, ChevronDown, ClipboardCheck, Menu, Search, Settings, Trophy, Users, CheckCircle2, Circle } from 'lucide-react';
import './styles.css';
import { RUBRIC, RUBRIC_TOTAL, emptyScores } from './data/rubric';
import { listDelegates, listCommissions, listEvaluations, saveEvaluation, setDelegateAttendance } from './lib/evaluations';
...
function App() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveEvaluation({ delegateId: selected.id, commissionId: commission, evaluatorId, scores, total: Number(total.toFixed(1)), comments });
      setSaved(true); setScores(emptyScores()); setCommission(''); setComments(''); setEvaluations(await listEvaluations());
      setTimeout(() => setSaved(false), 2500);
    } catch (err) { setError(err.message || 'No se pudo guardar la evaluación.'); }
  };
  const toggleAttendance = async delegate => {
    setError('');
    try {
      const updated = await setDelegateAttendance(delegate.id, !delegate.checked_in);
      setDelegates(current => current.map(item => item.id === delegate.id ? { ...item, checked_in: updated.checked_in } : item));
    } catch (err) { setError(err.message || 'No se pudo actualizar la asistencia.'); }
  };

  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="brand-mark">R17</div><div><strong>Regional 17</strong><span>Sistema de evaluación</span></div></div><div className="side-label">GESTIÓN</div><button className={page === 'checkin' ? 'nav-item active blue' : 'nav-item'} onClick={() => setPage('checkin')}><ClipboardCheck size={18}/> Pase de lista</button><button className={page === 'evaluation' ? 'nav-item active blue' : 'nav-item'} onClick={() => setPage('evaluation')}><Users size={18}/> Comisiones · Evaluación</button><button className={page === 'scoreboard' ? 'nav-item active purple' : 'nav-item'} onClick={() => setPage('scoreboard')}><Trophy size={18}/> ScoreBoard</button><div className="sidebar-bottom"><button className="nav-item"><Settings size={18}/> Configuración</button></div></aside>
    <main className="main"><header className="topbar"><button className="mobile-menu"><Menu size={20}/></button><div className="search"><Search size={17}/><span>Buscar delegado o NUID</span><kbd>⌘K</kbd></div><div className="top-actions"><div className="period">Etapa activa <span>22 ago — 03 sep</span><b>ACTIVA</b><ChevronDown size={15}/></div><div className="avatar">AS</div></div></header>{error && <div className="error-banner">{error}</div>}{page === 'evaluation' && <Evaluation {...{delegates, commissions, selected, setSelected, commission, setCommission, scores, updateScore, total, saved, handleSave, setScores, comments, setComments, evaluations, loading}} />}{page === 'scoreboard' && <Scoreboard evaluations={evaluations} />}{page === 'checkin' && <Checkin delegates={delegates} loading={loading} onToggle={toggleAttendance} />}</main>
  </div>;
}
...
function Checkin({ delegates, loading, onToggle }) {
  const [query, setQuery] = useState('');
  const filtered = delegates.filter(d => `${d.name} ${d.country} ${d.nuid || ''}`.toLowerCase().includes(query.toLowerCase()));
  const present = delegates.filter(d => d.checked_in).length;
  const pending = delegates.length - present;
  const percent = delegates.length ? Math.round((present / delegates.length) * 100) : 0;
  return <section className="content"><div className="breadcrumb">Inicio <span>·</span> <strong>Pase de lista</strong></div><div className="page-heading"><div><h1>Pase de lista</h1><p>Control de asistencia de los delegados de Regional 17.</p></div><span className="commission-pill">Asistencia · R17</span></div><div className="stats"><Stat icon={<Users/>} label="Delegados registrados" value={delegates.length} accent="blue"/><Stat icon={<CheckCircle2/>} label="Presentes" value={present} accent="purple"/><Stat icon={<Circle/>} label="Pendientes" value={pending} accent="amber"/></div><div className="card attendance-card"><div className="attendance-toolbar"><div><h2>Delegados</h2><p>{percent}% de asistencia registrada</p></div><div className="attendance-search"><Search size={15}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar por nombre, país o NUID..."/></div></div><div className="attendance-progress"><i style={{width:`${percent}%`}}/></div>{loading ? <div className="history-empty">Cargando delegados…</div> : filtered.length === 0 ? <div className="history-empty">No hay delegados que coincidan con la búsqueda.</div> : <div className="attendance-list">{filtered.map((delegate,index)=><div className="attendance-row" key={delegate.id}><div className="attendance-number">{String(index+1).padStart(2,'0')}</div><div className="attendance-avatar">{delegate.name.split(' ').map(x=>x[0]).slice(0,2).join('')}</div><div className="attendance-person"><strong>{delegate.name}</strong><span>{delegate.country} {delegate.nuid ? `· ${delegate.nuid}` : ''}</span></div><div className={`attendance-status ${delegate.checked_in ? 'present' : 'pending'}`}>{delegate.checked_in ? 'Presente' : 'Pendiente'}</div><button className={`attendance-button ${delegate.checked_in ? 'present' : ''}`} onClick={() => onToggle(delegate)}>{delegate.checked_in ? <><CheckCircle2 size={15}/> Presente</> : <><Circle size={15}/> Marcar presente</>}</button></div>)}</div>}</div></section>;
}
