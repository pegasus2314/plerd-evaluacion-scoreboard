function ensureCommissionStyles(){
  ...
  style.textContent=`
    .r17-commission-wrap{position:relative;margin-top:7px}
    .r17-commission-wrap .r17-commission-input{margin-top:0!important;width:100%!important;height:38px!important;padding:8px 34px 8px 10px!important;border-radius:9px!important;font-size:12px!important;box-sizing:border-box}
    ...
    .r17-commission-menu{position:absolute;left:0;right:0;top:calc(100% + 5px);display:none;max-height:190px;overflow:auto;background:#fff;border:1px solid #e2e7ef;border-radius:10px;box-shadow:0 12px 28px rgba(20,30,50,.12);padding:4px;z-index:260}
    .r17-commission-menu.open{display:block}
    .r17-commission-option{width:100%;border:0;background:transparent;text-align:left;border-radius:7px;padding:8px 9px;color:#344054;font-size:11px;cursor:pointer}
    .r17-commission-option:hover,.r17-commission-option.active{background:#eef4ff;color:#2f6fed}
...
function installCommissionAutocomplete(){
  ...
  document.querySelectorAll('label').forEach(label=>{
    if(!/^Comisión$/i.test(label.textContent.trim()))return;
    const select=label.querySelector('select');
    if(!select||select.dataset.commissionAutocomplete==='2')return;
    ...
    const render=()=>{
      ...
      const filtered=options().filter(o=>!query||o.textContent.toLocaleLowerCase('es').includes(query)).slice(0,12);
      menu.innerHTML=filtered.length
        ? filtered.map(o=>`<button type="button" class="r17-commission-option" data-value="${o.value.replace(/"/g,'&quot;')}">${o.textContent.trim()}</button>`).join('')
        : '<div class="r17-commission-empty">No hay coincidencias</div>';
      menu.querySelectorAll('.r17-commission-option').forEach(btn=>{
    ...
    input.addEventListener('focus',()=>{syncFromSelect();render();menu.classList.add('open')});
    ...
    document.addEventListener('mousedown',event=>{if(!wrap.contains(event.target))menu.classList.remove('open')});
    syncFromSelect();
  });
}
