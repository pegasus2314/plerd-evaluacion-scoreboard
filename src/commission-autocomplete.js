...
function labelText(label){return [...label.childNodes].filter(n=>n.nodeType===3).map(n=>n.textContent||'').join(' ').trim();}
...
function installAutocomplete(select){
  ...
  const wrap=document.createElement('div');wrap.className='r17-commission-wrap';select.parentNode.insertBefore(wrap,select);wrap.appendChild(select);select.classList.add('r17-commission-select-hidden');
  const input=document.createElement('input');input.type='text';input.className='r17-commission-input';input.autocomplete='off';input.placeholder='Buscar o seleccionar comisión';input.setAttribute('aria-label','Comisión');wrap.insertBefore(input,select);
  ...
  input.addEventListener('focus',()=>{render();menu.classList.add('open')});input.addEventListener('input',()=>{render();menu.classList.add('open')});
...
async function ensureDelegateCommissionField(){
  ...
  const label=document.createElement('label');label.className='r17-new-delegate-commission';label.innerHTML='<span>Comisión</span><select required><option value="">Cargando comisiones…</option></select><small class="r17-delegate-commission-help">La comisión se asignará al delegado y aparecerá automáticamente en la hoja de evaluación.</small>';
...
function captureDelegateSubmit(){document.addEventListener('submit',e=>{const form=e.target.closest?.('.modal form');if(!form)return;const field=form.querySelector('.r17-new-delegate-commission select');if(!field)return;pendingCommission={commissionId:field.value||null,name:form.querySelector('input[placeholder*="Albert"]')?.value?.trim()||'',country:form.querySelector('input[placeholder*="República"]')?.value?.trim()||'',nuid:form.querySelector('input[placeholder*="R17-"]')?.value?.trim()||'',at:Date.now()};setTimeout(savePendingCommission,700);setTimeout(savePendingCommission,1400);},true);}
async function savePendingCommission(){const p=pendingCommission;if(!p||Date.now()-p.at>5000||!p.commissionId)return;const mid=modelId();if(!mid)return;let q=supabase.from('scoreboard_delegates').select('id,commission_id').eq('model_id',mid).eq('nombre',p.name).eq('pais',p.country);q=p.nuid?q.eq('nuid',p.nuid):q.is('nuid',null);const {data}=await q.order('created_at',{ascending:false}).limit(1).maybeSingle();if(!data)return;if(data.commission_id!==p.commissionId){await supabase.from('scoreboard_delegates').update({commission_id:p.commissionId,updated_at:new Date().toISOString()}).eq('id',data.id);}pendingCommission=null;}

function findEvaluationSelectors(){
  const delegateSelect=[...document.querySelectorAll('select')].find(s=>/Delegado\/a/i.test(labelText(s.closest('label')||document.createElement('label'))));
...
function observeEvaluationSelection(){const {delegateSelect}=findEvaluationSelectors();if(delegateSelect&&!delegateSelect.dataset.r17CommissionSync){delegateSelect.dataset.r17CommissionSync='1';delegateSelect.addEventListener('change',()=>setTimeout(syncEvaluationCommission,0));}syncEvaluationCommission();}
...
captureDelegateSubmit();const observer=new MutationObserver(install);observer.observe(document.body,{childList:true,subtree:true});window.addEventListener('load',install);setTimeout(install,100);setTimeout(install,500);setTimeout(install,1200);setInterval(install,1000);
